...
NB: This page does not attempt to replace the [man page](http://www.openssh.org/manual.html) for pentesters, only to supplement it with some pertinent examples.
...
During a pentest or audit, you might want to add an authorized\_keys file to let you log in using an SSH key.

The authorized\_keys file lives in a user’s home directory on the SSH server.  It holds the public keys of the users allowed to log into that user’s account.

Generate a public/private key pair like this:

ssh-keygen -f mykey
cat mykey.pub # you can copy this to authorized\_keys
...
cat mykey.pub # copy to authorized\_key.  Omit the trailing user@host if you need a shorter key.
...
Caveat: The authorized\_keys file might not work if it’s writable by other users.  If you already have shell access you can “chmod 600 ~/.ssh/authorized\_keys”.  However, if you’re remotely exploiting an arbitrary file-write vulnerability and happen to have a weak umask, you may have problems.

## X11 Forwarding

If your SSH client is also an X-Server then you can launch X-clients (e.g. Firefox) inside your SSH session and display them on your X-Server.  This works well with from Linux X-Servers and from [cygwin](http://www.cygwin.com/)‘s X-server on Windows.
...
If you see SSH agents running on a pentest (process called “ssh-agent”), you might be able to use it to authenticate you to other hosts – or other accounts on that host.  Check out ~/.ssh/known\_hosts for some ideas of where you might be able to connect to.
...
export  SSH\_AUTH\_SOCK=/tmp/ssh-tqiEl28473/agent.28473
...
ps auxeww | grep ssh-agent | grep SSH\_AUTH\_SOCK | sed 's/.\*SSH\_AUTH\_SOCK=//' | cut -f 1 -d ' '
